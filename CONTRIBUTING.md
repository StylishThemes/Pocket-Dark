...
There are a number of ways to get involved with the development of this Pocket Dark theme. Even if you've never contributed to an Open Source project before, we're always looking for help identifying missing styles or other issues.
...
* This style does not have a size limit, but:
  * Don't add any image URI's to the CSS; instead add the image into the `/images` directory; then point to using the following url: `http://StylishThemes.github.io/Pocket-Dark/images/{my-image.png}`.
...
* [Download](https://github.com/StylishThemes/Pocket-Dark/archive/master.zip), [fork](https://github.com/StylishThemes/Pocket-Dark/fork) or clone this repository.
* Use [node.js](http://nodejs.org/) to run `npm install`.
* Make any changes to the `pocket-dark.user.css` file and save.

### Build & test

* Create & change into a new branch of your local Pocket-Dark repository.
...
* Now you can add and commit the changes of the `pocket-dark.user.css` file to your fork's branch.
